...
import type { Cadence, RecurringInputTransaction, RecurringSeries } from './types'
...
export function detectRecurring(transactions: RecurringInputTransaction[]): RecurringSeries[] {
  ...
  for (const [payeeNorm, txns] of groups) {
    ...
    series.push({
      payeeNorm, displayName: last.merchant, type: last.type === 'income' ? 'income' : 'expense',
      cadence, avgAmount, lastAmount: last.amount, previousAmount: previous?.amount ?? null,
      priceIncreased, lastDate: last.date, nextExpected, occurrences: sorted.length,
    })
  }
