          ...
          {links.map((l) => (
            <Link
              ...
              className={cn(
                ...
                pathname === l.to && l.to === '/features' && 'text-foreground',
              )}
            >
              {l.label}
            </Link>
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="w-72">
              ...
              <nav className="mt-6 flex flex-col gap-1" aria-label="Mobile">
                {links.map((l) => (
                  <Link key={l.label} href={l.to} onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 text-sm font-medium hover:bg-muted">
                    {l.label}
                  </Link>
                ))}
                <Link href="/sign-in" onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 text-sm font-medium hover:bg-muted">Sign in</Link>
              </nav>
            </SheetContent>
          </Sheet>
        </div>
            ...
            <div key={c.title}>
              ...
              <ul className="mt-3 space-y-2">
                {c.links.map(([label, to]) => (
                  <li key={label}><Link href={to} className="text-sm text-muted-foreground hover:text-foreground">{label}</Link></li>
                ))}
              </ul>
            </div>
          ))}
